...
func _comprobar_que_se_recorrio_todo(tabla: Dictionary) -> void:
	for seccion: String in ["mover", "dias", "pistas", "interseccion", "apuesta",
			"farol", "resumen", "ganadores"]:
...
func _cargar_tabla() -> Dictionary:
	if not FileAccess.file_exists(RUTA_TABLA):
		print("FALTA la tabla de paridad en %s (generala con `python3 terminal/paridad.py`)."
			% RUTA_TABLA)
		return {}
	...
	if int(tabla.get("formato", -1)) != FORMATO:
		print("La tabla de paridad es del formato %s y este script lee el %d."
...
func _test_constantes(c: Dictionary) -> void:
	...
	_afirmar_igual(
		TamborJuicio.DISPAROS_POR_DIA, int(c["disparos_por_dia"]), "DISPAROS_POR_DIA"
	)
	...
	_afirmar_igual(
		RuletaEstado.BONO_MARCA_ACERTADA,
		int(c["bono_marca_acertada"]),
		"BONO_MARCA_ACERTADA"
	)
	...
	for caso: Dictionary in casos:
		...
		if not coincide:
			_fallos.append(
				...
				% [tipo, posicion, ultimo, huecos, mentir, obtenido["texto"],
					obtenido["candidatos"], esperados]
			)
...
func _test_farol(casos: Array) -> void:
	for caso: Dictionary in casos:
		...
		_afirmar_igual(
			marca.marcas_restantes, int(caso["restantes"]), "farol %s (restantes)" % [caso["huecos"]]
		)
		_afirmar_igual(
