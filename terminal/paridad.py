...
try:
    from . import apuestas, estado, eventos, farol, historial, jugador, motor, pistas
...
# Los presets de dificultad viven en la interfaz de cada version
# (ruleta.DIFICULTADES y Dificultad.gd), asi que se declaran aqui para
# no hacer que este modulo importe la interfaz.
DIFICULTADES = {
...
def _apuesta() -> list[dict]:
    ...
    for guion in guiones:
        apuesta = apuestas.Apuesta(motor.APUESTA_BASE)
        devueltos = []
        for paso in guion:
            if paso == "doblar":
                devueltos.append(apuesta.doblar())
            elif paso == "bono":
                devueltos.append(apuesta.sumar_bono(motor.BONO_MARCA_ACERTADA))
            elif paso == "perder":
