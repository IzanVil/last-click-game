...
class TestSemilla(unittest.TestCase):
    def test_la_misma_semilla_juega_la_misma_partida(self):
        def cronica(semilla):
            juego = motor.Motor(semilla=semilla)
            pasos = [juego.tambor.patron, juego.tambor.posicion_bala]
            for numero in [1, 2, 3, 4, 5]:
                if juego.terminada:
                    break
                pasos.extend(_tipos(juego.disparar(numero)))
                pasos.append(juego.tambor.posicion_bala)
            return pasos

        self.assertEqual(cronica(4242), cronica(4242))
        self.assertGreater(len({tuple(cronica(s)) for s in range(8)}), 1)

    def test_sin_semilla_se_sortea_una_y_queda_apuntada(self):
        semillas_vistas = {motor.Motor().semilla for _ in range(12)}
        self.assertGreater(len(semillas_vistas), 1)
